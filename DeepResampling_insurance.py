...
def category_to_integer(category):
    if category == 'no':
        ...
        integer = 3
    elif category == 'southeast':
        integer = 4
    elif category == 'southwest':
        integer = 5
    elif category == 'northeast':
        integer = 6
    elif category == 'northwest':
        integer = 7
    else:
        integer = 8
    return(integer)
...
data['sex'] = data['sex'].map(category_to_integer) 
data['smoker'] = data['smoker'].map(category_to_integer) 
data['region'] = data['region'].map(category_to_integer) 
...
weights = [0.5, 0.5] 
hyperParam = [0.00, 1.00, 1.00, 0.00, 1.00, 0.00, 0.00]
hyperParam = hyperParam / np.sum(hyperParam)
flagParam = [0, 1, 1, 0, 1, 0, 1] 
...
loss_type = 'sum_loss'  # options: 'max_loss' or 'sum_loss' 
weights = [0.5, 0.5] 
hyperParam = [1.00, 0.00, 0.00, 1.00, 0.00, 1.00, 0.00]
hyperParam = hyperParam / np.sum(hyperParam)
flagParam = [1, 1, 1, 1, 1, 1, 1] 
history = deep_resampling(hyperParam, run, loss_type, n_batches, n_iter, 
                         nobs_synth, weights, flagParam, mode)
evaluate_and_save('synth_vg2.csv', weights, run, flagParam)
plot_history(history)
...
vg_scatter(dfs, features[0], features[6], 1)
vg_scatter(dfv, features[0], features[6], 2)
vg_scatter(dfs, features[0], features[2], 3)
vg_scatter(dfv, features[0], features[2], 4)
vg_scatter(dfs, features[6], features[2], 5)
vg_scatter(dfv, features[6], features[2], 6)
plt.show()
...
vg_histo(dfs, features[6], 2)
vg_histo(dfs, features[2], 3)
vg_histo(dfv, features[0], 4)
vg_histo(dfv, features[6], 5)
