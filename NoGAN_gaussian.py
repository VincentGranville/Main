...
# specify how generated points are distributed in each bin
# ... option 'Gaussian' can only be used with continuous features
# ... to use old version of NoGAN on feature k, set stretch[k] to - 1 
# ... do not use stretch[k] > 1 for categorical features
# ... one parameter value for each feature
# ... default: 'Uniform' for each feature with stretch[k] = 1.0

stretch_type = ['Gaussian', 'Gaussian', 'Gaussian', 'Uniform'] 
stretch = [1.0, 1.0, 1.0, 1.0]   

# specify how many points are generated in each bin
...
# now generate the points in each bin
...
for skey in bin_count:
    ...
    for i in range(count):        
        ...
        for k in range(n_features):
            if stretch[k] < 0:   # old version of NoGAN
                ...
                dist_to_edge = min(median[k] - L_bounds[k], U_bounds[k] - median[k])
                new_obs[k] = median[k] + dist_to_edge * stretch[k] * deviate
