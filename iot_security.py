...
data_C1_full = pd.merge(data_C1, data_B, how='inner', on=features)
data_C2_full = pd.merge(data_C2, data_B, how='inner', on=features)
print(data_C1_full)
print(data_C2_full)
data_C1_full.to_csv('iot_C1_full.csv')
data_C2_full.to_csv('iot_C2_full.csv')

map_C1 = data_C1_full.groupby('src_port')['size_y'].sum()
map_C2 = data_C2_full.groupby('src_port')['size_y'].sum()
